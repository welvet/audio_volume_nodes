...
class Preferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        col1 = row.split()
        col2 = row.split()
        col1.operator("audio_volume_nodes.install_dependencies")
        col2.operator("audio_volume_nodes.uninstall_dependencies", icon="CANCEL")
        layout.label(text='Check if librosa is installed (restart might be needed):')
        row2 = layout.row()
        row2.split().operator("scene.check_installation_audio_volume_nodes")
        row2.split().prop(self, "check_installation")


class InstallDependencies(bpy.types.Operator):
    bl_idname = "audio_volume_nodes.install_dependencies"
    ...
    def execute(self, context):
        ...
        try:
            ...
            f = open(os.path.join(site_packages, "test-temp-audioVolumeNodes.txt"), "w")
            f.close()
            os.remove(os.path.join(site_packages, "test-temp-audioVolumeNodes.txt"))
            access = True
        except:
            access = False
...
class UninstallDepencencies(bpy.types.Operator):
    bl_idname = "audio_volume_nodes.uninstall_dependencies"
    bl_label = "Remove"
...
class CheckInstallation(bpy.types.Operator):
    bl_idname = "scene.check_installation_audio_volume_nodes"
